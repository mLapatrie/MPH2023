...
def show_data(activation_values, destrieux_atlas_img):
    ...
    display = plotting.plot_stat_map(masked_img, display_mode='ortho', cut_coords=[0, -70, 0], title='Destrieux Atlas Activation', cmap='cividis')
    return display


def generate_frames(big_data, destrieux_atlas_img):
    frame_dir = 'frames'
    ...
    for i, activation_values in enumerate(big_data):
        ...
        display = show_data(activation_values, destrieux_atlas_img)
...
#big_data = np.load('example_predictions.npy')
#generate_frames(big_data, destrieux_atlas_img)